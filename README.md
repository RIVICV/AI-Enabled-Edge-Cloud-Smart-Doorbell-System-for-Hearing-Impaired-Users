...
| 成员 | 分工 | 职责 |
|------|------|------|
| **崔琇淇** | 云端与交互负责人 | 云平台配置、Flask 后端、MQTT 服务、SQLite 数据库、火山引擎 AI 对接、微信小程序全栈开发、紧急呼叫邮件通知 |
| **陈思** | 硬件通信负责人 | ESP-NOW 双板通信协议、MQTT 上传、硬件整体代码整合、离线 MQTT 队列缓存机制 |
| **黄佳翠** | 嵌入式驱动负责人 | WS2812 灯带 RMT 驱动、INMP441 麦克风 I2S 驱动、PIR 红外 GPIO 驱动、振动马达 PWM 驱动、SSD1306 OLED SPI 驱动、整体面包板搭建与集成测试 |
| **龚梁浩** | 边缘 AI 负责人 | ESP-DSP FFT 音频特征提取、TFLite Micro 三分类模型推理、按键+PIR+音频三通道融合判决算法、音频模型训练与量化部署 |
...
**团队成员**：感谢陈思、黄佳翠、龚梁浩三位队友的通力协作与付出。我们共同面对技术挑战、分享解决方案，每一次联调成功都离不开彼此的信任与配合。从硬件搭建到云端部署，从驱动调试到AI推理，每个环节都凝聚了团队的智慧与汗水。
